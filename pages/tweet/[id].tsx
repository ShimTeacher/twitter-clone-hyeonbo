...
const Tweet = () => {
  ...
  const { data, mutate } = useSWR<tweetResult>(
    typeof window === "undefined" && router.query.id !== undefined ? null : `/api/tweets/${router.query.id}`
  );
      ...
      <div className="p-4 bg-black border-b  border-gray-800">
        <div className="flex mt-5">
          ...
          <div className="flex-1 ml-3">
            <div className="flex flex-col">
              <span className="text-white">{data?.tweet.User.email}</span>
              <span className="text-white">@{data?.tweet.User.name}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 mb-3">
          ...
          <div className="flex gap-3 border-b border-gray-700 pb-4">
            <span className="text-gray-500">{data?.tweet?.createdAt}</span>
          ...
          <div className="flex gap-3 border-b border-gray-700 pb-4">
            ...
            <div className="flex justify-center items-center">
              <span className="text-white font-bold mr-1">{data?.tweet.Likes.length}</span>
        ...
        <div className="grid grid-cols-4">
          ...
          <div onClick={onFavClick} className="group flex gap-2 items-center justify-center cursor-pointer">
            <div className="flex justify-start items-center  group-hover:bg-[#181819] rounded-full p-1">
              {data?.isLiked ?
                <svg className="w-6 h-6 text-[#C91267]" fill="currentColor" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
